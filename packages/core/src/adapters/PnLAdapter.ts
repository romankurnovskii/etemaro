...
async function getMeteoraData(conn: Connection, walletAddress: string, flat: FlatPosition[]): Promise<Record<string, any>> {
  ...
  await Promise.all(
    [...positionsByPool.entries()].map(async ([pool, positionAddrs]) => {
      const cached = _meteoraCache.get(pool);
      const sigByPosition: Record<string, string | null> = {};
      await Promise.all(
        positionAddrs.map(async (addr) => {
...
function buildPosition(f: FlatPosition, prices: Record<string, number | null>, solUsd: number | null, meteora: any, solMode: boolean): BuiltPosition {
  ...
  if (pnlPctSuspicious) {
    log(
      'pnl_warn',
      `${f.position.slice(0, 8)} suspicious tick — priceMissing=${priceMissing} depositsMissing=${depositsMissing} (solUsd=${solUsd}, priceX=${priceX})`,
    );
  }

  const inRange =
    f.active != null && f.lower != null && f.upper != null ? f.active >= f.lower && f.active <= f.upper : meteora ? !meteora.isOutOfRange : true;
