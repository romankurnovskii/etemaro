...
function saveChatId(id: string): void {
  try {
    let cfg: Record<string, unknown> = fs.existsSync(USER_CONFIG_PATH)
...
export async function editMessageWithButtons(text: string, messageId: number, inlineKeyboard: InlineKeyboardButton[][]): Promise<TelegramApiResponse | null> {
  if (!TOKEN || !chatId || !messageId) return null;
  return postTelegram("editMessageText", {
    message_id: messageId,
    text: String(text).slice(0, 4096),
