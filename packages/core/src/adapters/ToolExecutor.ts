...
import {
  ...
  getPoolDetail,
  getTopCandidates,
} from "./blockchain/ScreeningAdapter.js";
import {
  getActiveBin,
  deployPosition,
  getMyPositions,
...
import {
  ...
  getTokenHolders,
  getTokenNarrative,
} from "./blockchain/TokenDataAdapter.js";

// ─── JS module imports (no type declarations) ─────────────────
import { addLesson, clearAllLessons, clearPerformance, removeLessonsByKeyword, getPerformanceHistory, pinLesson, unpinLesson, listLessons } from "../domain/lessons.js";
...
const toolMap: Record<string, ToolFn> = {
  ...
  list_lessons: ({ role, pinned, tag, limit }: Record<string, unknown> = {}) => listLessons({ role: role as string | null, pinned: pinned as boolean | null, tag: tag as string | null, limit: limit as number }),
  ...
  update_config: ({ changes, reason = "" }: Record<string, unknown>) => {
    ...
    for (const [key, val] of Object.entries(changes as Record<string, unknown>)) {
      ...
      if (!raw) { unknown.push(key); continue; }
 ...
 */
async function swapBaseToSolWithRetry(baseMint: string, label: string): Promise<{
  swapped: boolean;
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      log("executor", `Auto-swapping ${label} ${token.symbol || baseMint.slice(0, 8)} ($${(token.usd ?? 0).toFixed(2)}) back to SOL (attempt ${attempt}/${attempts})`);
  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && (result as any).tx) {
        notifySwap({ inputSymbol: (args.input_mint as string)?.slice(0, 8), outputSymbol: (args.output_mint as string) === "So11111111111111111111111111111111111111112" || (args.output_mint as string) === "SOL" ? "SOL" : (args.output_mint as string)?.slice(0, 8), amountIn: (result as any).amount_in, amountOut: (result as any).amount_out, tx: (result as any).tx }).catch(() => {});
      } else if (name === "deploy_position") {
        notifyDeploy({ pair: (result as any).pool_name || (args as any).pool_name || (args.pool_address as string)?.slice(0, 8), amountSol: (args.amount_y as number) ?? (args.amount_sol as number) ?? 0, position: (result as any).position, tx: (result as any).txs?.[0] ?? (result as any).tx, priceRange: (result as any).price_range, rangeCoverage: (result as any).range_coverage, binStep: (result as any).bin_step, baseFee: (result as any).base_fee }).catch(() => {});
      } else if (name === "close_position") {
        notifyClose({ pair: (result as any).pool_name || (args.position_address as string)?.slice(0, 8), pnlUsd: (result as any).pnl_usd ?? 0, pnlPct: (result as any).pnl_pct ?? 0 }).catch(() => {});
        ...
        if ((args.reason as string) && (args.reason as string).toLowerCase().includes("yield")) {
          ...
          if (poolAddr) addPoolNote({ pool_address: poolAddr, note: `Closed: low yield (fee/TVL below threshold) at ${new Date().toISOString().slice(0, 10)}` });
        }
        ...
        if (!args.skip_swap && (result as any).base_mint) {
          ...
          if (swapped) {
            ...
            (result as any).auto_swap_note = `Base token already auto-swapped back to SOL (${(result as any).base_mint.slice(0, 8)} → SOL). Do NOT call swap_token again.`;
 ...
 */
async function runSafetyChecks(name: string, args: Record<string, unknown>): Promise<{
  pass: boolean;
  reason?: string;
}> {
  switch (name) {
    case "deploy_position": {
      ...
      const alreadyInPool = (positions as any).positions.some(
        (p: any) => p.pool === args.pool_address
      );
      if (alreadyInPool) {
        return {
          pass: false,
      ...
      if (args.base_mint) {
        const alreadyHasMint = (positions as any).positions.some(
          (p: any) => p.base_mint === args.base_mint
        );
        if (alreadyHasMint) {
          return {
            pass: false,
