 ...
 * - Persists update_config changes to agent-config.json and restarts cron jobs when intervals change
 * - Logs every execution to the audit JSONL trail
 *
 * @dependencies node-cron
 * @sideEffects On-chain transactions via deploy/claim/close/swap; writes agent-config.json; sends Telegram notifications; starts child processes on self-update
 */
