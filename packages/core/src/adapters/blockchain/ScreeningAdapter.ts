...
import { getBlockedDevs, isDevBlocked } from '../../domain/dev-blocklist.js'
...
export function getRawPoolScreeningRejectReason(pool: RawPool, s: typeof config.screening): string | null {
  ...
  if (feeActiveTvlRatio == null || feeActiveTvlRatio < s.minFeeActiveTvlRatio) {
    return windowedFeeTvlRejectReason(
      feeActiveTvlRatio ?? Number.NaN,
...
function condensePool(p: RawPool): CondensedPool {
  return {
    ...
    volume_active_tvl_ratio:
      (p as any).volume_active_tvl_ratio != null ? fix((p as any).volume_active_tvl_ratio, 4) : null,
...
export async function getTopCandidates({ limit = 10 } = {}): Promise<TopCandidatesResult> {
  ...
  const filteredOut: FilteredExample[] = Array.isArray(discovery.filtered_examples)
...
export async function getPoolDetail({
  ...
  if (!pool) {
    throw new Error(
      `Pool ${pool_address} not found (not indexed in Pool Discovery API or DLMM API${discoveryError ? `: ${discoveryError}` : ''})`,
    )
  }
