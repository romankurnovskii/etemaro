...
function assertNoUnsafeSystemTransfer(
  ...
  for (const ix of getTransactionInstructions(tx)) {
    ...
    const decoded =
      type === 'Transfer' ? SystemInstruction.decodeTransfer(ix) : SystemInstruction.decodeTransferWithSeed(ix)
...
async function assertRangeDoesNotRequireBinArrayInitialization(
  pool: any,
  minBinId: number,
  maxBinId: number,
): Promise<void> {
  ...
  const accounts = await withRpcFailover((conn) => conn.getMultipleAccountsInfo(keys, 'confirmed'), {
  ...
  if (deriveBinArrayBitmapExtension && isOverflowDefaultBinArrayBitmap) {
    ...
    if (needsBitmapExtension) {
      ...
      const account = await withRpcFailover((conn) => conn.getAccountInfo(bitmapExtension, 'confirmed'), {
...
async function getPool(poolAddress: string): Promise<any> {
  ...
  if (!poolCache.has(key)) {
    ...
    const pool = await withRpcFailover((conn) => DLMM.create(conn, new PublicKey(poolAddress)), {
...
export async function deployPosition({
  ...
  const fallbackAmountY =
    amount_y == null && amount_sol == null ? computeDeployAmount((await getWalletBalances()).sol) : 0
  ...
  const actualBaseFee =
    base_fee ?? (baseFactor > 0 ? parseFloat((((baseFactor * actualBinStep) / 1e6) * 100).toFixed(4)) : null)

  const totalYLamports = new BN(Math.floor(finalAmountY * 1e9))
  let totalXLamports = new BN(0)
  if (finalAmountX > 0) {
    const mintInfo = await withRpcFailover((conn) => conn.getParsedAccountInfo(new PublicKey(pool.lbPair.tokenXMint)), {
    ...
    try {
      ...
      const matching =
        refreshed?.positions?.find(
          (position: any) =>
            position.pool === pool_address && position.lower_bin === minBinId && position.upper_bin === maxBinId,
...
function resolvePerformanceSignalSnapshot({
  poolAddress,
  baseMint,
  tracked,
}: {
...
async function _fetchRawOpenPositionsFromMeridian({
  walletAddress,
  agentId,
}: {
        ...
        for (const positionAddress of pool.listPositions || []) {
          ...
          const derivedPnlPct = lpData
            ...
            : binData
              ...
              : null
          const pnlPctDiff =
            reportedPnlPct != null && derivedPnlPct != null ? Math.abs(reportedPnlPct - derivedPnlPct) : null
          ...
          positions.push({
            ...
            unclaimed_fees_usd: lpData
              ? Math.round(
                  (config.management.solMode ? safeNum(lpData.unCollectedFeeNative) : safeNum(lpData.unCollectedFee)) *
                    10000,
                ) / 10000
              : binData
                ? Math.round(
            ...
            total_value_usd: lpData
              ? Math.round((config.management.solMode ? safeNum(lpData.valueNative) : safeNum(lpData.value)) * 10000) /
                10000
              : binData
                ? Math.round(
                    (config.management.solMode
            ...
            collected_fees_usd: lpData
              ? Math.round(
                  (config.management.solMode ? safeNum(lpData.collectedFeeNative) : safeNum(lpData.collectedFee)) *
                    10000,
                ) / 10000
              : binData
                ? Math.round(
            ...
            pnl_usd: lpData
              ? Math.round(
                  (config.management.solMode ? safeNum(lpData.pnl?.valueNative) : safeNum(lpData.pnl?.value)) * 10000,
                ) / 10000
              : binData
                ? Math.round(
                    parseFloat(config.management.solMode ? binData.pnlSol || 0 : binData.pnlUsd || 0) * 10000,
                  ) / 10000
                : null,
            pnl_true_usd: lpData
