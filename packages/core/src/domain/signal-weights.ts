...
const DEFAULT_WEIGHTS: Record<SignalName, number> = Object.fromEntries(SIGNAL_NAMES.map((s) => [s, 1.0])) as Record<
  SignalName,
  number
>
...
export function recalculateWeights(
  ...
  const weights: Record<SignalName, number> = (data.weights || { ...DEFAULT_WEIGHTS }) as Record<SignalName, number>
  ...
  const recent = perfData.filter((p) => {
    const ts =
      p.recorded_at || ((p as unknown as Record<string, unknown>).closed_at as string | undefined) || p.deployed_at
  ...
  for (const [signal, lift] of ranked) {
    const prev = weights[signal as SignalName]
    let next = prev
...
function extractNumeric(signal: SignalName, entries: PerformanceRecord[]): number[] {
  const vals: number[] = []
...
function getEntrySignalSnapshot(entry: PerformanceRecord): Record<string, unknown> | null {
  if (entry.signal_snapshot) return entry.signal_snapshot
  const snapshot: Record<string, unknown> = {}
  for (const signal of SIGNAL_NAMES) {
    if ((entry as unknown as Record<string, unknown>)[signal] != null)
      snapshot[signal] = (entry as unknown as Record<string, unknown>)[signal]
  }
