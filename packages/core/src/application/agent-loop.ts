      ...
      if (!msg.tool_calls || msg.tool_calls.length === 0) {
        ...
        if (mustUseRealTool && !sawToolCall) {
          ...
          logStructured({
            ...
            metadata: {
              retryCount: noToolRetryCount,
              maxRetries: 2,
              goal,
              rejectedContentSnippet: (msg.content || '').slice(0, 200),
            },
          })
      ...
      const orderedResults = msg.tool_calls.map((tc: any) => {
        if (blockedIds.has(tc.id)) {
          ...
          const blockedResult = {
            blocked: true,
            reason:
              'Only one deploy_position per message is allowed. If you need to deploy to multiple pools, send separate messages.',
          }
          onToolFinish?.({
