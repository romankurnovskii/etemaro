 ...
 * @description Zod schema based validator for runtime application configuration files (Version 7).
 *
 * @features
  ...
  if (!result.success) {
    ...
    const err: any = new Error(
      `${getConfigFileName()} has invalid or missing fields:\n${errorMessages}\n\n` +
        'Please ensure you are using the Version 7 schema structure.',
      { cause: result.error },
    )
    err.issues = issues
