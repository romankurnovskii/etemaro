...
if [ "$OS_NAME" = "darwin" ] && command -v brew >/dev/null 2>&1; then
    ...
    if brew info etemaro >/dev/null 2>&1; then
        echo "Installing Etemaro CLI via Homebrew..."
        brew install etemaro
    else
        echo "No official Homebrew formula yet. Falling back to npm..."
        install_via_npm
    fi
else
    install_via_npm
...
echo ""
echo "Etemaro also has a cross-platform desktop app (Tauri/React)."
...
case "$REPLY" in
    [yY]|[yY][eE][sS])
        ...
        ;;
    *)
        echo "Skipping desktop app. You can install later from:"
        echo "  https://github.com/romankurnovskii/etemaro-desktop/releases"
        ;;
esac
        ...
        darwin)
            if command -v brew >/dev/null 2>&1; then
                ...
                if brew info --cask etemaro >/dev/null 2>&1; then
                    ...
                    echo "Or build from source: https://github.com/romankurnovskii/etemaro-desktop"
                fi
            else
