...
**Meridian CLI (use `node --import tsx packages/cli/src/Cli.ts <cmd>`):**
...
The Meridian TS codebase only supports **single-sided SOL-only LPing** (depositing SOL in bins below the current price to DCA-buy the token as it dips, earning fees). Swapping to the base token and depositing token-only (amount_x) or double-sided liquidity is disabled.
...
```
*Note: `--bins-above` MUST always be 0, and `--amount-x` is not supported (keep at 0 or omit).*
...
4. **Calculate Deploy Amount:**
   ...
   - *Ensure the final amount is greater than the minimum deploy threshold (usually 0.1 SOL).*
