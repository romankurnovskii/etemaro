...
```
              ...
              └───────────┬────────────┘     in JSON config values
                          ▼
              ┌──────────────────────────────┐
              │ instances/agent-default.json │ ─── Default agent config (or custom
              └───────────┬──────────────────┘     via AGENT_CONFIG_PATH env var)
                          ▼
              ┌────────────────────────┐
...
This keeps secrets out of the JSON file — ideal for API keys (`jupiter.apiKey`, `gmgn.apiKey`, `hiveMind.apiKey`, `api.publicApiKey`) and environment-specific URLs (`rpcUrl`, `pnl.rpcUrl`). Wallets are referenced by alias via `"wallet": "<alias>"` and stored securely in the keystore.
